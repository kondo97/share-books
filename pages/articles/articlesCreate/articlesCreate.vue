<template>
  <v-row class="justify-center ma-3">
      ...
      <v-card color="#C8D7D0" class="create-card">
        ...
        <div class="py-3">
          ...
          <div class="text-center">
            <v-btn elevation="2" color="success" @click="nextBook" :disabled="!validCreate"
              >反映する</v-btn
            >
          </div>
        </div>
    ...
    <v-col cols="12" sm="6" class="px-sm-3 px-md-12 create-border right-create">
      ...
      <h1 class="mt-6" :class="{ articleTitle: articleTitle }">{{ articleTitle }}</h1>
      <p class="mt-6">
        {{ articleDescript }}
      </p>
      <v-row class="mt-3">
...
export default {
  ...
  methods: {
    nextBook() {
      ...
      this.$store.dispatch('articlesCreate/pushBook', this.create)
      this.create = {
        title: "",
        author: "",
        descript: "",
  ...
  computed: {
    contents() {
      return this.$store.getters['articlesCreate/contents']
    }
  }
};
</script>
